...
import com.telefonica.gal.RTG_SPAIN_TD_HELPDESK.interfaceWs.topplus.WsTopPlus;
import com.telefonica.gal.RTG_SPAIN_TD_HELPDESK.interfaceWs.wsMiView.WsMiViewTv;
...
public class FactoryTD<T> {
    ...
    public GetProfileResponse invokeWs(T routingTD, T request, Map<String, Object> hashMap) {
        GetProfileResponse response = new GetProfileResponse();
        List<InvokeWsFactory> invokeWsList = getInvokeWs(routingTD, request, hashMap);

        if (invokeWsList.size() == 1) {
            return (GetProfileResponse) invokeWsList.get(0).getInvokeWs().invoke();
        }
        ...
        for (InvokeWsFactory invokeWs : invokeWsList) {
            if (!invokeWs.isSynchronous() && invokeWs.getType().equals("source")) {
                ...
                asyncFactory.start();
                return (GetProfileResponse) invokeWs.getInvokeWs().invoke();
            }

            if (invokeWs.getType().equals("source")) {
                response = (GetProfileResponse) invokeWs.getInvokeWs().invoke();
            } else {
