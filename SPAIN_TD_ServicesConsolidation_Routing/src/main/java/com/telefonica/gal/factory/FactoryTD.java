...
import com.telefonica.gal.interfaceWs.WsMiViewTv;
import com.telefonica.gal.interfaceWs.WsTopPlus;
...
public class FactoryTD<T> {
    ...
    public SERVICESCONSOLIDATIONRESPONSE invokeWs(T routingTD, T request, Map<String, Object> hashMap) {
        SERVICESCONSOLIDATIONRESPONSE response = new SERVICESCONSOLIDATIONRESPONSE();

        List<InvokeWsFactory> invokeWsList = getInvokeWs(routingTD, request, hashMap);

        if (invokeWsList.size() == 1) {
            return (SERVICESCONSOLIDATIONRESPONSE) invokeWsList.get(0).getInvokeWs().invoke();
        }
        ...
        for (InvokeWsFactory invokeWs : invokeWsList) {
            if (!invokeWs.isSynchronous() && invokeWs.getType().equals("source")) {
                ...
                asyncFactory.start();
                return (SERVICESCONSOLIDATIONRESPONSE) invokeWs.getInvokeWs().invoke();
            }

            if (invokeWs.getType().equals("source")) {
                response = (SERVICESCONSOLIDATIONRESPONSE) invokeWs.getInvokeWs().invoke();
            } else {
